...
REM ====================================================================
REM  WinDimmer64 MSIX build script
REM
REM  Usage:
REM    build_msix.bat
REM
REM  Prerequisites:
REM    - WinDimmer64.exe built (run build.bat or LLVM-MinGW commands)
...
REM
REM  Outputs:
REM    - dist\WinDimmer64_1.4.7.0_x64.msix       (unsigned, for Store)
REM    - dist\WinDimmer64_1.4.7.0_x64.msix      (self-signed, for sideload)
REM    - dist\WinDimmer64_1.4.7.0_x64.cer       (signing cert, for sideload)
REM ====================================================================
...
REM --------------------------------------------------------------------
if not exist "%PROJECT_ROOT%\WinDimmer64.exe" (
    echo [ERROR] WinDimmer64.exe not found in project root.
...
copy /y "%PROJECT_ROOT%\WinDimmer64.exe" "%STAGE_DIR%\WinDimmer64.exe" >nul
...
%SDK_BIN%\makeappx.exe pack /v /o /d "%STAGE_DIR%" /p "%DIST_DIR%\WinDimmer64_%VERSION%_%ARCH%.msix"
if errorlevel 1 (
    echo [ERROR] makeappx pack failed.
    exit /b 1
)
echo       Created: %DIST_DIR%\WinDimmer64_%VERSION%_%ARCH%.msix
echo.

REM --------------------------------------------------------------------
REM  Step 5: Generate self-signed cert and sign (for sideloading only)
REM --------------------------------------------------------------------
set CERT_PFX=%DIST_DIR%\WinDimmer64_SelfSign.pfx
set CERT_CER=%DIST_DIR%\WinDimmer64_SelfSign.cer
set CERT_SUBJECT="CN=UnDadFeated"
...
%SDK_BIN%\signtool.exe sign /f "%CERT_PFX%" /p wddim64 /fd SHA256 /tr http://timestamp.digicert.com /td SHA256 "%DIST_DIR%\WinDimmer64_%VERSION%_%ARCH%.msix"
if errorlevel 1 (
    echo [WARN] signtool signing failed. The unsigned MSIX is still valid for Store submission.
) else (
    echo       Signed: %DIST_DIR%\WinDimmer64_%VERSION%_%ARCH%.msix
)

echo.
echo === Build complete ===
echo.
echo   Store submission (unsigned):  %DIST_DIR%\WinDimmer64_%VERSION%_%ARCH%.msix
echo   Sideloading (signed):         %DIST_DIR%\WinDimmer64_%VERSION%_%ARCH%.msix
