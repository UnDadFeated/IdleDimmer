...
- Output: `WinDimmer64.exe` (~150 KB), `WinDimmer64-Setup-v1.4.7.exe` (~280 KB with embedded exe), `dist\WinDimmer64_1.4.7.0_x64.msix` (~880 KB, for Microsoft Store)

## Microsoft Store (MSIX) Build

- **MSIX** (Desktop Bridge package of `WinDimmer64.exe`): `msix\build_msix.bat` — requires Windows 10 SDK 10.0.22621 (`winget install --id=Microsoft.WindowsSDK.10.0.22621 -e --silent`)
  ...
  - **Output**: `dist\WinDimmer64_1.4.7.0_x64.msix` (~880 KB)
  - **Cert** (sideload only): `dist\WinDimmer64_SelfSign.pfx` / `.cer` (password: `wddim64`)
