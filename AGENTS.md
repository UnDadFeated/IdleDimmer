...
- **LLVM-MinGW** (always rebuild `resources.o` and `setup_res.o`):
  ...
  llvm-windres resources\resources.rc -O coff -o resources\resources.o
   clang++ -O2 -std=c++17 -mwindows -Os -s -mguard=cf -o IdleDimmer.exe src\main.cpp src\MainWindow.cpp src\DimmerManager.cpp src\ConfigManager.cpp resources\resources.o -lgdi32 -ld2d1 -ldwrite -ldwmapi -lole32 -luuid -lwinhttp -Wl,--dynamicbase -Wl,--nxcompat -Wl,--high-entropy-va
  ```
- **Setup** (requires `IdleDimmer.exe` in project root — rebuild resources *after* app exe to embed the correct version):
   ...
   clang++ -O2 -std=c++17 -mwindows -Os -s -mguard=cf -o IdleDimmer-Setup-v1.4.7.exe src\setup.cpp resources\setup_res.o -lole32 -lshell32 -ladvapi32 -luuid -lcomctl32 -lversion -ldwmapi -Wl,--dynamicbase -Wl,--nxcompat -Wl,--high-entropy-va

- Output: `IdleDimmer.exe` (~150 KB), `IdleDimmer-Setup-v1.4.7.exe` (~280 KB with embedded exe), `dist\IdleDimmer_1.4.7.0_x64.msix` (~880 KB, for Microsoft Store)
...
- **MSIX** (Desktop Bridge package of `IdleDimmer.exe`): `msix\build_msix.bat` — requires Windows 10 SDK 10.0.22621 (`winget install --id=Microsoft.WindowsSDK.10.0.22621 -e --silent`)
  ...
  - **Output**: `dist\IdleDimmer_1.4.7.0_x64.msix` (~880 KB)
...
2. **Execute Headless Release**: Set the token to the `GH_TOKEN` environment variable so `gh` uses it directly, bypassing scope verification login limits, and run the release create command:
   ...
   $env:GH_TOKEN="<retrieved_token>"
    gh release create v1.4.7 IdleDimmer-Setup-v1.4.7.exe --title "v1.4.7" --notes "Release notes go here"
   ```
